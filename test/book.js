...
import ZipJsArchive from '../src/core/zipjs-archive';
import assert from 'assert';

describe('Book', function() {
	const zipjs = globalThis.zip;
	...
	describe('Archived epub in array buffer with zipjs backend', function() {
		let book;

		before(async function() {
			const response = await fetch("/fixtures/alice.epub");
			const buffer = await response.arrayBuffer()
			book = new Book(buffer, { archiveMethod: "zipjs", zipjs })
		})
		...
		it('should open a archived epub', async function() {
			...
			assert.equal(book.isOpen, true, "book is opened");
			assert(book.archive, "book is unarchived");
			assert(book.archive instanceof ZipJsArchive, "zipjs archive backend is used");
		});
