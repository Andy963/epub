...
class PdfBook {
	...
	async open(input) {
		...
		async loadMetadata() {
			if (!this.pdf || typeof this.pdf.getMetadata !== "function") {
				this.loading.metadata.resolve(this.package && this.package.metadata ? this.package.metadata : {});
				return;
			}
			...
			try {
				...
				const metadata = extend(this.package && this.package.metadata ? this.package.metadata : {}, {
		...
		hrefFromPageIndex(pageIndex) {
			const index = typeof pageIndex === "number" && isFinite(pageIndex) ? Math.floor(pageIndex) : 0;
			const section = this.spine && typeof this.spine.get === "function" ? this.spine.get(index) : undefined;
	...
	section(target) {
		...
		async getPageText(pageNumber) {
			...
			const page = typeof pageNumber === "number" && isFinite(pageNumber) ? Math.floor(pageNumber) : 1;
			...
			entry.promise = this.pdf.getPage(page)
		...
		async searchText(query, options) {
			...
			const findMatches = (text) => {
				...
				while (matches.length < maxResultsPerSection && lastIndex <= haystack.length) {
					const index = haystack.indexOf(needle, lastIndex);
					if (index === -1) {
						break;
					}
		...
		async renderPageData(pageNumber) {
			const pageIndex = typeof pageNumber === "number" && isFinite(pageNumber) ? Math.floor(pageNumber) : 1;
			...
			if (this.settings.annotationLayer) {
				try {
					...
					annotationLayer = await this.buildAnnotationLayerHtml(annotations, cssViewport);
				} catch (e) {
					annotationLayer = "";
				}
			}
		...
		buildTextLayerHtml(textContent, viewport) {
			const items = textContent && Array.isArray(textContent.items) ? textContent.items : [];
			const pdfjs = this.pdfjsLib();
			const transform = pdfjs && pdfjs.Util && typeof pdfjs.Util.transform === "function"
